/* COSY English Pronunciation Marathon Shared Engine */
...
function visualGrapheme(v) {
  ...
  return svgWrap(w, h, `
    ...
    ${wordEls}
    <text x="${w / 2}" y="202" text-anchor="middle" font-size="12" fill="${MP_PALETTE.inkSoft}" letter-spacing="1">EXAMPLES</text>
  `);
}
...
function visualAlphabet() {
  ...
  return svgWrap(w, h, `
    <text x="${w / 2}" y="32" text-anchor="middle" font-style="italic" font-size="13" fill="${MP_PALETTE.inkSoft}">26 letters, 44 sounds: letter names vs letter sounds</text>
    ${els}
  `);
}
...
function visualTree(v) {
  const w = 640, h = 260;
  const title = v.title || "Phonetic Decision Tree";
  const root = v.root || "Question?";
  const left = v.left || { label: "Option A", result: "Result A", color: MP_PALETTE.green };
  const right = v.right || { label: "Option B", result: "Result B", color: MP_PALETTE.red };
...
function initMarathonProgress() {
  ...
  const STORAGE_KEY = "cosy_en_marathon_progress";
  let progress = {};
  try {
  ...
  const updateUI = () => {
    ...
    if (countLabel) countLabel.textContent = `${count} / ${total} lessons`;
  };
  ...
  document.querySelectorAll(".lesson-sample").forEach((sample, idx) => {
    ...
    if (header && !header.querySelector(".lesson-check")) {
      ...
      checkBtn.textContent = isSeen ? "✓ Read" : "+ Mark as read";
      if (isSeen) checkBtn.style.background = "#ffffff", checkBtn.style.color = "#1e3a8a";

      checkBtn.addEventListener("click", () => {
        if (pageSeen.includes(lessonId)) {
          pageSeen = pageSeen.filter(id => id !== lessonId);
          checkBtn.textContent = "+ Mark as read";
...
function initUnitTabs() {
  ...
  tabsNav.style.cssText = "display:flex; gap:0.4rem; overflow-x:auto; padding:0.5rem 1.5rem; background:#f0f4f8; border-bottom:1px solid #e2e8f0; scrollbar-width:none;";

  const allBtn = document.createElement("button");
  allBtn.className = "mp-unit-tab active";
  allBtn.textContent = "🔍 View All";
  allBtn.style.cssText = "font-weight:700; font-size:0.82rem; padding:0.3rem 0.8rem; border-radius:20px; border:1px solid #1e3a8a; background:#1e3a8a; color:#fff; cursor:pointer; white-space:nowrap;";
  ...
  sections.forEach((sec, idx) => {
    ...
    btn.style.cssText = "font-weight:700; font-size:0.82rem; padding:0.3rem 0.8rem; border-radius:20px; border:1px solid #cbd5e1; background:#fff; color:#1e3a8a; cursor:pointer; white-space:nowrap;";
  ...
  allBtn.addEventListener("click", () => {
    ...
    allBtn.style.background = "#1e3a8a";
