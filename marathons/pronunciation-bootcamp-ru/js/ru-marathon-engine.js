/* COSY Russian Pronunciation Marathon Shared Engine */
...
function visualGrapheme(v) {
  ...
  return svgWrap(w, h, `
    ...
    ${wordEls}
    <text x="${w / 2}" y="202" text-anchor="middle" font-size="12" fill="${MP_PALETTE.inkSoft}" letter-spacing="1">ПРИМЕРЫ</text>
  `);
}
...
function visualAlphabet() {
  ...
  let els = rows.map((row, ri) => {
    ...
    return letters.map((l, i) => {
      const x = 24 + i * 36;
      const y = 80 + ri * 60;
      return `<text x="${x}" y="${y}" text-anchor="middle" font-size="18" font-weight="700" fill="${ri === 0 ? MP_PALETTE.red : MP_PALETTE.ink}">${l}</text>`;
    }).join("");
  }).join("");

  return svgWrap(w, h, `
    <text x="${w / 2}" y="32" text-anchor="middle" font-style="italic" font-size="13" fill="${MP_PALETTE.inkSoft}">33 буквы: 10 гласных, 21 согласная, 2 знака (ь, ъ)</text>
    ${els}
  `);
}
...
function visualTree(v) {
  const w = 640, h = 260;
  const title = v.title || "Фонетическое дерево решений";
  const root = v.root || "Вопрос?";
  const left = v.left || { label: "Да", result: "Результат А", color: MP_PALETTE.green };
  const right = v.right || { label: "Нет", result: "Результат Б", color: MP_PALETTE.red };
...
function initMarathonProgress() {
  ...
  const STORAGE_KEY = "cosy_ru_marathon_progress";
  let progress = {};
  try {
  ...
  const updateUI = () => {
    ...
    if (countLabel) countLabel.textContent = `${count} / ${total} уроков`;
  };
  ...
  document.querySelectorAll(".lesson-sample").forEach((sample, idx) => {
    ...
    if (header && !header.querySelector(".lesson-check")) {
      ...
      checkBtn.textContent = isSeen ? "✓ Прочитано" : "+ Отметить прочитанным";
      if (isSeen) checkBtn.style.background = "#ffffff", checkBtn.style.color = "#991b1b";

      checkBtn.addEventListener("click", () => {
        if (pageSeen.includes(lessonId)) {
          pageSeen = pageSeen.filter(id => id !== lessonId);
          checkBtn.textContent = "+ Отметить прочитанным";
          checkBtn.style.background = "rgba(255,255,255,0.2)";
          checkBtn.style.color = "#ffffff";
        } else {
          pageSeen.push(lessonId);
          checkBtn.textContent = "✓ Прочитано";
          checkBtn.style.background = "#ffffff";
          checkBtn.style.color = "#991b1b";
        }
...
function initUnitTabs() {
  ...
  tabsNav.style.cssText = "display:flex; gap:0.4rem; overflow-x:auto; padding:0.5rem 1.5rem; background:#fef2f2; border-bottom:1px solid #fee2e2; scrollbar-width:none;";

  const allBtn = document.createElement("button");
  allBtn.className = "mp-unit-tab active";
  allBtn.textContent = "🔍 Показать все";
  allBtn.style.cssText = "font-weight:700; font-size:0.82rem; padding:0.3rem 0.8rem; border-radius:20px; border:1px solid #991b1b; background:#991b1b; color:#fff; cursor:pointer; white-space:nowrap;";
  ...
  sections.forEach((sec, idx) => {
    ...
    btn.style.cssText = "font-weight:700; font-size:0.82rem; padding:0.3rem 0.8rem; border-radius:20px; border:1px solid #fca5a5; background:#fff; color:#991b1b; cursor:pointer; white-space:nowrap;";
  ...
  allBtn.addEventListener("click", () => {
    ...
    allBtn.style.background = "#991b1b";
    allBtn.style.color = "#fff";
    allBtn.style.borderColor = "#991b1b";
